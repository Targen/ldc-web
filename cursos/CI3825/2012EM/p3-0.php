...
** glibc detected ** ./rautomake: malloc(): memory corruption
                        ...
                        <li id="p14">
                                <h3>Uso no estructurado de <code>fork</code></h3>
                                <h4>Pregunta</h4>
                                <blockquote>
                                        <ol>
                                                <li><p>Mira, si no hago el fork, el programa me dice cuales son los archivos del nivel en el que estoy y me dice cuales son los directorios. Si hago el fork, me da la misma información, repitiéndola muchas veces y después se queda pegado.</p></li>
                                                <li><blockquote>
<pre><code><![CDATA[
...
función recursiva)
                                                ...
                                                <li><p>Qué estoy haciendo mal? T_T</p></li>
                                        </ol>
                                </blockquote>
                                <h4>Respuesta</h4>
                                <ol>
                                        <li><p>Cuando ejecutan <code>fork</code>, el proceso se duplica y continúa su ejecución en exactamente el mismo código en exactamente el mismo lugar, con (casi) exactamente los mismos descriptores de archivo y (casi) el mismo entorno. Las diferencias exactas entre el padre y el hijo están enumeradas en la página de manual de <code>fork</code>, pero la que les interesa es básicamente que el retorno de <code>fork</code> es diferente (0 en el hijo, y el PID del hijo en el padre).</p></li>
                                        <li><p>Lo típico que se hace luego de hacer <code>fork</code> es obligar a que el hijo se vaya por un camino de ejecución totalmente diferente al del padre y que nunca vayan a terminar ejecutando las mismas cosas. En esencia es algo así:</p></li>
                                        <li><blockquote>
<pre><code><![CDATA[
/* código del padre */
switch (fork()) {
...
/* el resto del código del padre */
                                        ...
                                        <li><p>Lo más importante es el segundo <code>exit</code>: el hijo nunca debería salir de ahí y continuar su flujo de ejecución por donde pasó el padre.</p></li>
                                        <li><p>Esto no es una regla universal en el sentido de que sea necesario para que las cosas funcionen, pero es extremadamente difícil escribir código que funcione bien y no trabaje así. Por ejemplo, este es un programa en C que imprime los números del 1 al 1000 en un orden no determininístico usando forks no estructurados:</p></li>
                                        <li><blockquote>
<pre><code><![CDATA[
#include <unistd.h>
#include <stdio.h>
